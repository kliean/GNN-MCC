...
def run(task_name = "informative", alpha=0.2, beta=0.5, gamma=0.2, l1=0.2):
    ...
    def train(model, epoch, loader):
        ...
        for batch in batches:
            ...
            print(f"e_acc:{avg_edge_acc:.4f}, n_acc:{avg_node_acc:.4f}")
    ...
    for epoch in range(args.epoch):
        ...
        # test_loss, test_acc, test_loss_list, _, _ = evaluate(model, dev_loader, epoch, test=True)
        # print(f'---test_loss:{test_loss:.4f}, test_acc:{test_acc:.4f}---')
        if best_loss > eval_loss:
            best_loss = eval_loss
            best_epoch = epoch
            # save best loss
            # torch.save(model.state_dict(), os.path.join(save_path, "models-loss-{:.4f}.pth".format(best_loss)))
            # torch.save(optimizer.state_dict(), os.path.join(save_path, "optimizer-loss-{:.4f}.pth".format(best_loss)))
