...
def run(task_name = "informative", alpha=0.2):
    ...
    def train(model, epoch, loader):
        ...
        for batch in batches:
            ...
            h1, h2, attn, t= ContrastiveAttentionFusion(text, image)
            ...
            with torch.no_grad():
                h1, h2, _, _ = ContrastiveAttentionFusion(text, image)
...
if __name__ == '__main__':
    start_t = time.time()
    acc, report, cm, evaluation = run(args.task)
    end_t = time.time()
