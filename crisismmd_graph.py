...
dataset_dir = '/home/mnt/xxx/datasets/CrisisMMD'
data_image_dir = os.path.join(dataset_dir, "CrisisMMD_v2.0")
use_agreed_label = args.use_agreed_label

# clip_model = "D:/models/transformers\clip-ViT-B-32"
clip_model = "/opt/PLMs/clip-ViT-B-32"
...
if use_agreed_label:
    ...
    data_splits_dir = os.path.join(dataset_dir, "CrisisMMD_v2.0", "crisismmd_datasplit_all")
    task_labels = {
        "humanitarian": [
            "affected_individuals",
            "infrastructure_and_utility_damage",
...
def encode(data_splits):
    ...
    mm_encoder = SentenceTransformer(clip_model)
    embed = {}
...
def dataset_to_graph(task, data_splits, embed, mode='tra-dev', edge_type='a', tau=0.8):
    messages = f'task: {task}, edge_type: {edge_type}, tau: {tau}, data_splits: {data_splits.keys()}\n'
    print(f'task: {task}, edge_type: {edge_type}, tau: {tau}, data_splits: {data_splits.keys()}')
    messages += f'num of nodes: {len(embed)}\n'
    print(f'num of nodes: {len(embed)}')
    os.makedirs(cache_path, exist_ok=True)
    if use_agreed_label:
    ...
    #     data, mapping, label_to_id = pickle.load(open(crisis_file, "rb"))
    ...
    edges = torch.zeros((len(x), len(x)), dtype=torch.bool)

    att_edges = torch.zeros((len(x), len(x)), dtype=torch.bool)

    similarity_matrix = torch.mm(norm_x, norm_x.t())
    print(f'range of the similarity matrix:{similarity_matrix.min()}-{similarity_matrix.max()}')
    messages += f'range of the similarity matrix:{similarity_matrix.min()}-{similarity_matrix.max()}\n'

    use_selfloops = True
    directed = True
    print(f'use self loops: {use_selfloops}')
    messages += f'use self loops: {use_selfloops}\n'
    print(f'directed: {directed}')
    messages += f'directed: {directed}\n'
    if edge_type == 's':
        sem_edges = similarity_matrix > tau
        if ~use_selfloops:
            sem_edges.fill_diagonal_(False)
        print('num of semantic edges:', sem_edges.sum())
        messages += f'num of semantic edges:{sem_edges.sum()}\n'
        edges = sem_edges
    elif edge_type == 'a':
        all_items = [item for lst in data_splits.values() for item in lst]
        attr_edges_ = build_attr_graph(all_items, mapping)
        ...
        print('num of attr edges: ', att_edges.sum())
        messages += f'num of attr edges:{att_edges.sum()}\n'
        edges = att_edges
    elif edge_type == 'a_s':
        sem_edges = similarity_matrix > tau
        ...
        if ~use_selfloops:
            sem_edges.fill_diagonal_(False)
            att_edges.fill_diagonal_(False)
        print(f'num of semantic edges: {sem_edges.sum()}')
        messages += f'num of semantic edges:{sem_edges.sum()}\n'
        print('num of attr edges: ', att_edges.sum())
        messages += f'num of attr edges:{att_edges.sum()}\n'
        edges = sem_edges | att_edges
        print('num of all edges: ', edges.sum())
        messages += f'num of all edges:{edges.sum()}\n'
    else:
        messages += f"Invalid edges_type: {edge_type}. Expected 'a', 's', or 'a_s'."
    ...
    for split_name, split_data in data_splits.items():
        ...
        messages += f'split name:{split_name}\n'
    ...
    print(f'num agreed: {num_agreed}')
    messages += f'num of agreed: {num_agreed}\n'
    if ~use_selfloops:
        edges.fill_diagonal_(False)
    print(f'total of edges after add text-img within a tweet: {edges.sum()}')
    messages += f'total of edges after add text-img within a tweet: {edges.sum()}\n'

    if ~directed:
        edges = torch.triu(edges)
    ...
    print('num of training nodes: ', data.node_train_mask.sum())
    messages += f'num of training nodes: {data.node_train_mask.sum()}\n'
    print('num of dev-ing nodes: ', data.node_dev_mask.sum())
    messages += f'num of dev-ing nodes: {data.node_dev_mask.sum()}\n'
    print('num of testing nodes: ', data.node_test_mask.sum())
    messages += f'num of testing nodes: {data.node_test_mask.sum()}\n'
    print('num of training pairs', data.pairs_train.shape[-1])
    messages += f'num of training pairs: {data.pairs_train.shape[-1]}\n'
    print('num of dev-ing pairs', data.pairs_dev.shape[-1])
    messages += f'num of dev-ing pairs: {data.pairs_dev.shape[-1]}\n'
    print('num of testing pairs', data.pairs_test.shape[-1])
    messages += f'num of testing pairs: {data.pairs_test.shape[-1]}\n'
    with open(crisis_file, "wb") as f:
        pickle.dump((data, mapping, label_to_id), f)
        print('data saved:',crisis_file)
        messages += f'data saved:{crisis_file}\n'

    log_file = f'log/{task}-{use_agreed_label}-{edge_type}-{tau}-{mode}-{use_selfloops}-{directed}.txt'
